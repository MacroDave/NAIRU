...
model {
  //priors
  	//NAIRU
  	NAIRU[1] ~ normal(6,1);  //inital value of 'true' GDP
...
//  	 lambda_pu ~ normal(0,1); //coef on UR change
 // 	 eps_pu ~ normal(0,1); //variance of ULC eqn error term  
  ...
  //NAIRU
	...
	//INFLATION
	for(t in 4:T) {
		Y[t,4] ~ normal(.35*Y[t,5] 
						+ .24*Y[t-1,4] 
						+ .16*Y[t-2,4]
						+ .18*Y[t-3,4]
						+ .06*Y[t-1,1]
						- .38*(Y[t,3]-NAIRU[t])/Y[t,3]
						- .7*(Y[t-1,3]-Y[t-2,3])/Y[t,3]
						+ .004*(Y[t-1,4]-Y[t-2,4])
						, 0.3);
	}
	 ...
	 //ULC
	 for(t in 4:T) {
		Y[t,1] ~ normal(0.45*Y[t,5] 
						+ .47*Y[t-1,4] 
						+ .09*Y[t-2,4]
						- 1.9*(Y[t,3]-NAIRU[t])/Y[t,3]
						- 5.6*(Y[t-1,3]-Y[t-2,3])/Y[t,3]
						, 1.17);
	}
}
